package com.example.controller;

import java.time.LocalDateTime;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.example.entity.FxDeal;
import com.example.service.FxDealService;

import lombok.extern.slf4j.Slf4j;

@RestController
@RequestMapping("api/deals")
@Slf4j
public class FxDealController {

	
	@Autowired
	private FxDealService dealService;
	
	@PostMapping("/saveDeal")
    public ResponseEntity<String> createDeal(@RequestBody FxDeal deal) {
		log.info("Entry :: createDeal() ::: FxDealController.java");
        try {
        	if (deal.getDealTimestamp() == null) {
                deal.setDealTimestamp(LocalDateTime.now());
            }
        	log.info("FxDealController.java :: Received request to save deal Details :: ");
        	dealService.saveDeal(deal);
            return new ResponseEntity<>("Deal saved successfully", HttpStatus.CREATED);
        } catch (IllegalArgumentException e) {
        	log.info("Exit :: createDeal() ::: FxDealController.java");
            return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
        }
    }
}
